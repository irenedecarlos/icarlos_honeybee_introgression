...
for (Rep in 1:nRep) {
  ...
  for (year in 1:nYear) {
    ...
    print(Sys.time())
    #selection on queen values qg vignetes
    ...
    newrow1<- data.frame(MeanIBD=mean(colonyRecords[(uno+1):dos,15]), VarIBD=var(colonyRecords[(uno+1):dos,15]), Year=year,Rep=Rep,Population="Mel"
                         , HoneyYield=mean(colonyRecords[(uno+1):dos,13]), Fitness=mean(colonyRecords[(uno+1):dos,14]),Homocigosity=mean(colonyRecords[(uno+1):dos,12]))
    ...
    newrow2<- data.frame(MeanIBD=mean(colonyRecords[(dos+1):tres,15]), VarIBD=var(colonyRecords[(dos+1):tres,15]), Year=year,Rep=Rep, Population="Car"
                         , HoneyYield=mean(colonyRecords[(dos+1):tres,13]), Fitness=mean(colonyRecords[(dos+1):tres,14]),Homocigosity=mean(colonyRecords[(dos+1):tres,12]))
    #Combine what we had in each dataframe with the new info, so each year the dataframe updates eith new values
